...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void handleRegisterSuccess(AuthResult result) {
        showToast(getString(R.string.register_success_check_email));

        getSharedPreferences("auth", MODE_PRIVATE)
                .edit()
                .putString("pending_profile_name", etName.getText().toString().trim())
                .putString("pending_profile_email", etEmail.getText().toString().trim())
                .apply();

        if (authService.isUserLoggedIn()) {
            authService.updateUser(result.getUser())
                    .thenAccept(r -> {})
